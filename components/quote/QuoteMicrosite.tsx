...
import { ArrowDown, Check, CheckCircle2, Cog, FileText, Gauge, ShieldCheck, Timer, Wrench, Zap } from "lucide-react";
...
export function QuoteMicrosite({ quote, token, companyEmail, companyPhone }: { quote: PublicQuote; token: string; companyEmail?: string; companyPhone?: string }) {
  ...
  const remaining = formatTimeRemaining(quote.valid_until, now);
  const recipient = quote.lead.company ? `${quote.lead.full_name} · ${quote.lead.company}` : quote.lead.full_name;
  ...
  return <div className="bg-[#f0f4f5] text-[#102b39]">
    <section className="relative isolate overflow-hidden bg-[#06141d] text-white">
      ...
      <div className="container-page relative grid min-h-[660px] items-end gap-8 pb-12 pt-16 lg:grid-cols-[1.02fr_.98fr] lg:pb-16 lg:pt-24">
        <div className="max-w-2xl">
          <div className="flex flex-wrap items-center gap-3"><span className="rounded-full border border-cyan-200/20 bg-cyan-300/10 px-3 py-1 font-mono text-[10px] tracking-[.18em] text-cyan-100">PROPUESTA TÉCNICA / {quote.quote_number}</span><StatusPill status={quote.status} /><span className="rounded-full border border-white/10 bg-white/[.04] px-3 py-1 font-mono text-[10px] tracking-[.14em] text-slate-300">{quote.items.length} {quote.items.length === 1 ? "EQUIPO" : "EQUIPOS"}</span></div>
          ...
          <div className="mt-6 flex max-w-xl items-start gap-3 rounded-xl border border-cyan-200/15 bg-cyan-300/[.07] p-4"><AwardMark /><div><p className="font-mono text-[9px] tracking-[.16em] text-[#78e1ff]">PREPARADA PARA</p><p className="mt-1 text-sm font-bold text-white">{recipient}</p><p className="mt-1 text-xs text-slate-300">Configuración comercial y técnica elaborada para su operación.</p></div></div>
          ...
          <div className="mt-7 flex flex-wrap items-center gap-4"><button onClick={() => navigate("equipo")} className="flex items-center gap-2 text-sm font-bold text-white/80 transition hover:text-[#68dcff]"><span className="grid h-9 w-9 place-items-center rounded-full border border-white/20"><ArrowDown className="h-4 w-4" /></span>Conoce la solución</button>{isOpen && remaining && <div className="rounded-lg border border-amber-200/20 bg-amber-300/[.08] px-3 py-2"><p className="font-mono text-[9px] tracking-[.13em] text-amber-200">TIEMPO PARA ACEPTAR</p><p className="mt-1 text-sm font-black text-white">{remaining}</p></div>}</div>
        </div>
    ...
    {quote.items.length > 1 && <section className="container-page pt-10"><div className="mb-4 flex items-end justify-between"><div><p className="font-mono text-[10px] tracking-[.16em] text-brand-blue">ALCANCE DEL SUMINISTRO</p><h2 className="mt-1 text-2xl font-black tracking-tight text-brand-navy">Una solución compuesta por {quote.items.length} equipos.</h2></div><p className="hidden max-w-xs text-right text-xs leading-5 text-slate-500 sm:block">Selecciona un equipo para recorrer su ficha técnica en esta propuesta.</p></div><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{quote.items.map((entry, index) => <button key={`${entry.product_name}-${index}`} onClick={() => setActiveIndex(index)} className={`group overflow-hidden rounded-2xl border text-left transition ${activeIndex === index ? "border-[#27bde9] bg-white shadow-[0_10px_25px_rgba(8,105,139,.16)]" : "border-[#d4e2e6] bg-white hover:-translate-y-1 hover:border-[#93d6e9] hover:shadow-lg"}`}><ProductVisual name={entry.product_name} material={entry.material} capacity={entry.capacity_value} compact className="h-32" /><div className="p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-mono text-[9px] tracking-[.14em] text-brand-blue">EQUIPO {String(index + 1).padStart(2, "0")}</p><p className="mt-1 font-bold text-brand-navy">{entry.product_name}</p></div>{activeIndex === index && <Check className="h-4 w-4 text-[#079bc9]" />}</div><p className="mt-2 text-xs text-slate-500">{entry.product_model} · {entry.capacity_value} {entry.capacity_unit}</p><p className="mt-1 text-xs text-slate-400">{entry.material} · {entry.power || "Configuración estándar"}</p></div></button>)}</div></section>}
...
function AwardMark() { return <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full border border-cyan-200/20 bg-cyan-300/10 text-[#76e2ff]"><Check className="h-4 w-4" /></span>; }
function formatTimeRemaining(validUntil: string | null, now: number) { if (!validUntil) return null; const difference = new Date(validUntil).getTime() - now; if (difference <= 0) return "vigencia finalizada"; const hours = Math.floor(difference / 3_600_000); const minutes = Math.floor((difference % 3_600_000) / 60_000); return `${hours} h ${minutes} min`; }
